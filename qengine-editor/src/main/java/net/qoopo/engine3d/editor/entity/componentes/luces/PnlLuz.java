 ...
 */
package net.qoopo.engine3d.editor.entity.componentes.luces;
...
//                    source = spnNormalAmount;
    ...
    private void populateLightControl(QLigth luz) {
        try {
            lock = true;
            ...
            if (luz instanceof QSpotLigth || luz instanceof QPointLigth) {
...
//                luz.setRadioSombra(Float.parseFloat(txtMapaSombraRadio.getText()));
                if (luz instanceof QSpotLigth) {
                    ((QSpotLigth) luz).setAnguloExterno((float) Math.toRadians(getFloatFromSpinner(spnAngCono)));
                    ((QSpotLigth) luz).setAnguloInterno((float) Math.toRadians(getFloatFromSpinner(spnAngCono2)));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
