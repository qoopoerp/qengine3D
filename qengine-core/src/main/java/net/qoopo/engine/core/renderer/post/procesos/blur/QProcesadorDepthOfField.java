...
public class QProcesadorDepthOfField extends QPostProceso {
    ...
    private QTextura transposedHBlur(QTextura buffer) {
        ...
        // if (
        // (tipo == DESENFOQUE_LEJOS && buffer.getZBuffer(x, y) / buffer.getMaximo() >=
        // distanciaFocal)
        // || (tipo == DESENFOQUE_CERCA && buffer.getZBuffer(x, y) / buffer.getMaximo()
        // <= distanciaFocal)
        // || (tipo == DESENFOQUE_EXCLUYENTE && Math.abs(buffer.getZBuffer(x, y) /
        // buffer.getMaximo() - distanciaFocal) > margen)) {
        // pixel.set(1, 0, 0, 0);
