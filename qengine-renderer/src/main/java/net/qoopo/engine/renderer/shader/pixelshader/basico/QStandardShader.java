 ...
 */
package net.qoopo.engine.renderer.shader.pixelshader.basico;
 ...
 * @author alberto
 */
public class QStandardShader extends QShader {
    ...
    public QStandardShader(RenderEngine render) {
        super(render);
    }
                ...
                for (QLigth luz : render.getLitgths()) {
                    ...
                    if (luz != null && luz.entity.isToRender() && luz.isEnable()) {
                        ...
                        if (proc != null && render.opciones.isSombras() && material.isSombrasRecibir()) {
                            factorSombra = proc.factorSombra(QTransformar.transformarVectorInversa(
                                    pixel.ubicacion.getVector3(), pixel.entity, render.getCamara()), pixel.entity);
                        }
                        ...
                        if (luz instanceof QPointLigth || luz instanceof QSpotLigth) {
                            ...
                            vectorLuz.set(pixel.ubicacion.getVector3().clone().subtract(
                                    QTransformar.transformarVector(QVector3.zero, luz.entity, render.getCamara())));
                            ...
                            QColor colorLuz = QMath.calcularColorLuz(color, colorEspecular, luz.color,
                                    luz.energia * factorSombra * ao, pixel.ubicacion.getVector3(),
                                    vectorLuz.invert().normalize(), pixel.normal, material.getSpecularExponent(),
                                    reflectancia);
                            // atenuacion
                            ...
                            iluminacion.getColorLuz()
                                    .addLocal(QMath.calcularColorLuz(color, colorEspecular, luz.color,
                                            luz.energia * factorSombra * ao, pixel.ubicacion.getVector3(),
                                            vectorLuz.invert().normalize(), pixel.normal,
    ...
    private void calcularEntorno(QPixel pixel) {
        ...
        if (render.opciones.isMaterial()
            ...
            try {
                ...
                tm.vector3f2.set(QTransformar.transformarVectorNormal(
                        QTransformar.transformarVectorNormalInversa(pixel.normal, pixel.entity, render.getCamara()),
                ...
                tm.vector3f1.set(QTransformar.transformarVector(
                        QTransformar.transformarVectorInversa(pixel.ubicacion, pixel.entity, render.getCamara()),
