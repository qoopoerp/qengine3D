...
public class QTransformar {
     ...
     * @return
     */
    public static QVertexBuffer transformar(Entity entity, Camera camara, QVertexBuffer transformado) {
        // List<Entity> nueva = new ArrayList<>();
        // nueva.add(entity);
        transformar(entity, camara.getMatrizTransformacion(QGlobal.tiempo).invert(), transformado);
        return transformado;
    }
     ...
     * @return
     */
    public static QVertexBuffer transformar(Entity objeto, QMatriz4 matrizVista, QVertexBuffer transformado) {
        ...
        //logger.info("  Transformar-- Tiempo init=" + (System.currentTimeMillis() - ti));
            ...
            for (EntityComponent componente : objeto.getComponents()) {
                if (componente instanceof Mesh) {
                    ...
                    for (QVertex vertice : ((Mesh) componente).vertices) {
                        transformado.setVertice(QVertexShader.transform(vertice, matVistaModelo), nVertices);
                        nVertices++;
                    }
    ...
    public static void recalcularNormales(QVertexBuffer transformado) {
        ...
        // } catch (Exception e) {
        // }
        //logger.info("  Transformar-- Tiempo recalcular normales=" + (System.currentTimeMillis() - ti));
    }
