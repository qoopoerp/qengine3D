 ...
 * @author alberto
 */
public class PBRFragmentShader extends FragmentShader {

    public PBRFragmentShader(RenderEngine render) {
        super(render);
    }
    ...
    public QColor shadeFragment(Fragment fragment, int x, int y) {
        ...
        if (material.getMapaColor() == null || !render.opciones.isMaterial()) {
            ...
            if (!material.getMapaColor().isProyectada()) {
                ...
                // ya calculadas
                colorBase = material.getMapaColor().getQColor(fragment.u, fragment.v);
            } else {
                ...
                colorBase = material.getMapaColor().getQColor((float) x / (float) render.getFrameBuffer().getAncho(),
                        -(float) y / (float) render.getFrameBuffer().getAlto());
            }

        }
        computeNormal(fragment);
        computeLighting(fragment, color, colorBase, corregirAlbedo);
        return color;
    }
     ...
     * @param fragment
     */
    protected void computeLighting(Fragment fragment, QColor color, QColor colorBase, boolean corregirAlbedo) {
        ...
        try {
            ...
            if (render.opciones.isMaterial() && material.getMapaMetalico() != null) {
                metalico = material.getMapaMetalico().getQColor(fragment.u, fragment.v).r;
            }
            ...
            try {
                ...
                tm.vector3f2.set(TransformationVectorUtil.transformarVectorNormal(
                        TransformationVectorUtil.transformarVectorNormalInversa(pixel.normal, pixel.entity,
                                render.getCamara()),
                        pixel.entity.getMatrizTransformacion(QGlobal.time)));
     ...
     * @return
     */
    private QColor corregirGamma(QColor color) {
        QVector3 tmpColor = color.rgb();
        // HDR tonemapping
        tmpColor.divide(tmpColor.clone().add(QVector3.unitario_xyz));
        // Con ajuste de exposicion
        // tmpColor = QVector3.unitario_xyz.clone().subtract(QVector3.of((float)
        // Math.exp(-tmpColor.x * exposicion), (float) Math.exp(-tmpColor.y *
        // exposicion), (float) Math.exp(-tmpColor.z * exposicion)));
        // correccion de gamma
        tmpColor.set(QMath.pow(tmpColor.x, exponenteGamma), QMath.pow(tmpColor.y, exponenteGamma),
                QMath.pow(tmpColor.z, exponenteGamma));
        // cambia el color
