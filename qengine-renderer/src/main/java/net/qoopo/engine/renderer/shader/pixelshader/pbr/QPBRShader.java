 ...
 */
package net.qoopo.engine.renderer.shader.pixelshader.pbr;
...
//            QVector3 V = QTransformar.transformarVector(QTransformar.transformarVectorInversa(pixel.ubicacion, pixel.entity, render.getCamara()), pixel.entity).getVector3();
...
//            V.invert();//  Cam-WordPos
            ...
            QVector3 N2 = (QTransformar.transformarVectorNormal(QTransformar.transformarVectorNormalInversa(pixel.normal, pixel.entity, render.getCamara()), pixel.entity.getMatrizTransformacion(QGlobal.tiempo)));
            N2.normalize();
                ...
                for (QLigth luz : render.getLitgths()) {
                    ...
                    if (luz != null && luz.entity.isToRender() && luz.isEnable()) {
                        ...
                        if (proc != null && render.opciones.isSombras() && material.isSombrasRecibir()) {
                            sombra = proc.factorSombra(QTransformar.transformarVectorInversa(pixel.ubicacion, pixel.entity, render.getCamara()).getVector3(), pixel.entity);
                        }
                        float alfa = 0.0f;
                        if (luz instanceof QPointLigth || luz instanceof QSpotLigth) {
                            vectorLuz.set(pixel.ubicacion.getVector3().clone().subtract(QTransformar.transformarVector(QVector3.zero, luz.entity, render.getCamara())));
...
//                        float attenuation = 1.0f / (distanciaLuz * distanciaLuz);
                        ...
                        if (luz instanceof QSpotLigth) {
                            ...
                            radiacion.multiply(QMath.clamp((alfa - ((QSpotLigth) luz).getAnguloExterno()) / Math.min(((QSpotLigth) luz).getAnguloInterno() - ((QSpotLigth) luz).getAnguloExterno(), -0.0001f), 0.0f, 1.0f));
                        }
...
//                QVector3 specular = colorReflejo.rgb().multiply(kS);
                ...
                //                QVector3 ambient = QVector3.of(0.03f, 0.03f, 0.03f).multiply(albedo.clone().multiply(iluminacion.getColorAmbiente().rgb())).multiply(factorSombraSAO);
                //                QVector3 ambient = QVector3.of(0.03f, 0.03f, 0.03f).multiply(color.rgb()).multiply(factorSombraSAO);
...
//                          tm.vector3f2.set(pixel.normal);
                tm.vector3f2.set(QTransformar.transformarVectorNormal(QTransformar.transformarVectorNormalInversa(pixel.normal, pixel.entity, render.getCamara()), pixel.entity.getMatrizTransformacion(QGlobal.tiempo)));
...
//                tm.vector3f1.set(currentPixel.ubicacion.getVector3());                
                tm.vector3f1.set(QTransformar.transformarVector(QTransformar.transformarVectorInversa(pixel.ubicacion, pixel.entity, render.getCamara()), pixel.entity).getVector3());
                //ahora restamos la posicion de la camara a la posicion del mundo
