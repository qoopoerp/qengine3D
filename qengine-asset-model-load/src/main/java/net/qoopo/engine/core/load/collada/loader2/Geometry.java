...
public class Geometry extends Entity {
    ...
    public Geometry(String id, int verticesCount, String verticesData) {
        ...
        this.vertices = new Vector4[verticesCount];
        parseVerticesData();
    }
    ...
    private void parseTriangleData() {
        ...
        for (int i = 0; i < trianglesCount; i++) {
            Vector4 a = vertices[Integer.parseInt(parsedData[i * (offset * 3) + (0 * offset)])];
            Vector4 b = vertices[Integer.parseInt(parsedData[i * (offset * 3) + (1 * offset)])];
            Vector4 c = vertices[Integer.parseInt(parsedData[i * (offset * 3) + (2 * offset)])];
