...
//ejemplod ela araña
//        this.armatureData = visualSceneNode.getChild("visual_scene").getChildWithAttribute("node", "id", "Spider_Armature");
...
//        matrix.set(convertData(matrixData).array());
        matrix.set(convertDataArray(matrixData));
//        matrix.transpose();
        if (isRoot) {
...
//            Matrix4f.mul(CORRECTION, matrix, matrix);
    ...
    private float[] convertDataArray(String[] rawData) {
        float[] matrixData = new float[16];
        for (int i = 0; i < matrixData.length; i++) {
            matrixData[i] = Float.parseFloat(rawData[i]);
        }
