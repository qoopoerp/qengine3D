...
public class Geometry extends Entity {
    ...
    public Geometry(String id, int verticesCount, String verticesData) {
        ...
        this.vertices = new QVector4[verticesCount];
        parseVerticesData();
    }
    ...
    private void parseVerticesData() {
        ...
        for (int i = 0; i < vertices.length; i++) {
            QVector4 p = new QVector4();
            p.x = (float) Double.parseDouble(parsedData[i * 3 + 2]);
            p.y = (float) Double.parseDouble(parsedData[i * 3 + 1]);
            p.z = (float) -Double.parseDouble(parsedData[i * 3 + 0]);
    ...
    private void parseTriangleData() {
        ...
        for (int i = 0; i < trianglesCount; i++) {
            QVector4 a = vertices[Integer.parseInt(parsedData[i * (offset * 3) + (0 * offset)])];
            QVector4 b = vertices[Integer.parseInt(parsedData[i * (offset * 3) + (1 * offset)])];
            QVector4 c = vertices[Integer.parseInt(parsedData[i * (offset * 3) + (2 * offset)])];
